...
prefix = "/home/russellm/rllab/data/local/4WayComplex-Test-CorrelatedNoise-seed"
...
for itr in range(56,100):
    percent, Image = task(prefix, itr)
    plt.imshow(Image)
    plt.title("Success Rate: "+str(percent)+" percent" )
    percentHistory.append(percent)

    fobj = open("complex4state_metaLearned/Records_56onward.csv", "wb")
    
    pickle.dump(percentHistory, fobj)
    fobj.close()
    
    plt.savefig("complex4state_metaLearned/itr_"+str(itr)+".png")
    plt.clf()
